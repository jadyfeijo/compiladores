...
int isFuncallValid(ASTREE* root, ASTREE* node) // implementar - implementado
{
    if((root->type == ASTREE_FUNDEC_PAR || root->type == ASTREE_FUNDEC_VOID) && strcmp(root->son[1]->symbol->text,node->son[0]->symbol->text)==0)
    {
		...
		ASTREE *param_aux = node->son[1];
		ASTREE *decl_aux = root->son[2];
        ...
        while(declaration_param_list!=NULL && funcall_param_list!=NULL){
			...
			param_aux->symbol = decl_aux->son[1]->symbol;
			//fprintf(stderr,"\n>>>>>>>>>> %d %s\n", param_aux->type, param_aux->symbol->text);
			...
			//node->son[1]->symbol = root->son[2]->son[1]->symbol; //////////////
			//fprintf(stderr,"\nASDGHJASKDLS:ADSALDHSADJKL %d %s\n",node->type, declaration_param_list->symbol->text);

			//fprintf(stderr,"FUNDEC PAR %s",declaration_param_list->son[1]->symbol);
			//funcall_param_list->symbol = declaration_param_list->son[1]->symbol; ///////////// decl ta certo
			

            if (declaration_param_list->son[2])
			{
			decl_aux = decl_aux->son[2]->son[0];
			...
			param_aux = param_aux->son[1]->son[0];
